...
**The Global Footer verification corpus.** This is the most valuable single use of the live exemplars. Our `fragments/global-footer.html` is the highest-stakes normative fragment we maintain and it is currently unverified. Rather than trusting one source, `sws prior-art verify-footer` can fetch the rendered footer from all 17 exemplars, extract the link sets, and report the consensus plus any outliers. `www.stanford.edu` is the canonical reference among them. A link set that agrees across seventeen shipped Stanford sites is far stronger evidence than a documentation page, and the outliers are informative in their own right, since they are either drift to report or legitimate variants we do not know about.
