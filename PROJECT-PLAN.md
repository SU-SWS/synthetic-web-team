...
**Releases automated, and the MCP package namespaced, 8 September 2026.** Every push to `main` now cuts a release through `.github/workflows/release.yml`: patch by default, `[minor]` or `[major]` in the commit message to go further, `[skip release]` to cut nothing. Three things are worth recording because none of them is obvious.

`scripts/set-version.mjs` exists rather than `npm version` because `@su-sws/synthetic-web-team-mcp` depends on the root package with a caret range. A patch bump survives that; a minor one does not — `^0.1.0` does not match `0.2.0` — so a published server would have quietly resolved *old* standards. Silent, shipped, and invisible in testing. The script moves all four packages in lockstep and rewrites the range.

The MCP server was `@su-sws/mcp`, which claimed the generic name for the whole scope and would have foreclosed `@su-sws/decanter-mcp` and anything like it. Now `@su-sws/synthetic-web-team-mcp`, pairing with the root package. The bin stays `sws-mcp`: npm resolves a package's single bin regardless of its name, verified rather than assumed.

Authentication is **trusted publishing over OIDC**, so there is no npm token in the repository. npm caps granular tokens at 90 days and has been retiring non-expiring automation tokens as part of its supply-chain hardening, which made a stored secret both a recurring chore and a standing liability. The cost is a coupling worth knowing about: the trust names this repository *and the workflow's filename*, so renaming `release.yml` breaks publishing with an error that does not mention filenames.
